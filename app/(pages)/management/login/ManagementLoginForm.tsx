...
import { requestManagementLinkAction } from './actions'
...
export default function ManagementLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setSubmitting(true)
    await requestManagementLinkAction(email)
    setSubmitting(false)
    setSent(true)
  }
  ...
  if (sent) {
    return (
      <div className="bg-card border border-border rounded-lg p-6 text-sm text-foreground/80">
        If that email matches a management company, a sign-in link has been
